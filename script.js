...
document.addEventListener('DOMContentLoaded', () => {
  // ЗАГЛУШКА: Принудительный выход из аккаунта при каждом запуске
  // УДАЛИТЬ ЭТУ СТРОКУ КОГДА СКАЖУ
...
async function initializePushNotifications() {
  try {
    ...
    currentToken = await messaging.getToken({
      vapidKey: 'YOUR_VAPID_KEY_HERE' // Замените на ваш VAPID ключ
    });
...
async function sendAdminNotification(orderData) {
  try {
    ...
    // Отправляем уведомление через Firebase Cloud Functions
    // (это нужно будет настроить на сервере)
    const response = await fetch('/api/send-notification', {
      ...
      body: JSON.stringify({
        ...
        notification: {
          title: '🚕 Новый заказ!',
          body: `${orderData.from} → ${orderData.to} (${orderData.price} ₽)`,
          icon: '/logo.png'
        },
        data: {
