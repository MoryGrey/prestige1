...
function showOrderPage() {
  ...
  // Инициализируем промокод ПРЕСТИЖ
  initializePrestigePromoCode();
}

// Функция инициализации промокода ПРЕСТИЖ
async function initializePrestigePromoCode() {
  try {
    const promoDoc = await firebase.firestore().collection('promocodes').doc('ПРЕСТИЖ').get();
    
    if (!promoDoc.exists) {
      // Создаем промокод ПРЕСТИЖ
      await firebase.firestore().collection('promocodes').doc('ПРЕСТИЖ').set({
        code: 'ПРЕСТИЖ',
        discount: 50,
        maxActivations: 10,
        activations: 0,
        ...
        description: 'Скидка 50₽ на поездку (максимум 1 раз на пользователя, всего 10 активаций)'
      });
      
      console.log('Промокод ПРЕСТИЖ создан');
    }
  } catch (error) {
    console.error('Ошибка инициализации промокода:', error);
  }
}

// Функция для просмотра статистики промокода в админ панели
async function showPromoStats() {
  try {
    const promoDoc = await firebase.firestore().collection('promocodes').doc('ПРЕСТИЖ').get();
    
    if (!promoDoc.exists) {
      showNotification('Промокод ПРЕСТИЖ не найден', 'error');
      return;
    }
    ...
    usersSnapshot.forEach(doc => {
      ...
      users.push({
        id: doc.id,
        name: userData.userName || 'Неизвестно',
        usedAt: userData.usedAt ? userData.usedAt.toDate().toLocaleString('ru-RU') : 'Неизвестно'
      });
    });
    ...
    modal.innerHTML = `
      ...
      ">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
          <h2 style="margin: 0; color: #FFD600;">📊 Статистика промокода ПРЕСТИЖ</h2>
        ...
        <div style="margin-bottom: 20px;">
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
            <div style="background: #222; padding: 15px; border-radius: 10px; text-align: center;">
              <div style="color: #FFD600; font-size: 1.2rem; font-weight: bold;">${promoData.activations}</div>
              <div style="color: #ccc; font-size: 0.9rem;">Использовано</div>
            </div>
            <div style="background: #222; padding: 15px; border-radius: 10px; text-align: center;">
              <div style="color: #4CAF50; font-size: 1.2rem; font-weight: bold;">${promoData.maxActivations}</div>
              <div style="color: #ccc; font-size: 0.9rem;">Лимит</div>
            </div>
          </div>
          
          <div style="background: #222; padding: 15px; border-radius: 10px; text-align: center;">
            <div style="color: ${promoData.activations >= promoData.maxActivations ? '#F44336' : '#4CAF50'}; font-size: 1.2rem; font-weight: bold;">
              ${promoData.activations >= promoData.maxActivations ? 'Исчерпан' : 'Активен'}
            </div>
            <div style="color: #ccc; font-size: 0.9rem;">Статус</div>
          </div>
        </div>
        
        <div>
          <h3 style="color: #FFD600; margin-bottom: 15px;">👥 Пользователи, использовавшие промокод:</h3>
          ${users.length > 0 ? `
              ...
              ${users.map(user => `
                ...
                ">
                  <div>
                    ...
                    <div style="color: #ccc; font-size: 0.8rem;">ID: ${user.id}</div>
                  </div>
                  <div style="color: #999; font-size: 0.8rem;">${user.usedAt}</div>
                </div>
              `).join('')}
            </div>
          ` : `
            <div style="text-align: center; color: #ccc; padding: 20px;">
              Пока никто не использовал промокод
            </div>
          `}
        </div>
      </div>
    ...
    console.error('Ошибка загрузки статистики промокода:', error);
...
function updateOrderPrice() {
  ...
  if (priceValue) {
    if (activePromoCode && promoDiscount > 0) {
      const originalPrice = totalPrice + promoDiscount;
      priceValue.innerHTML = `<span style="text-decoration: line-through; color: #ccc; font-size: 0.9em;">${originalPrice} ₽</span> <span style="color: #4CAF50;">${totalPrice} ₽</span>`;
    } else {
...
function createOrderCard(order) {
  ...
  if (order.promoCode && order.promoDiscount > 0) {
    promoInfo = `
      <div class="history-item-promo" style="margin-top: 5px; font-size: 0.8rem; color: #4CAF50;">
        Промокод: ${order.promoCode} (-${order.promoDiscount}₽)
      </div>
    `;
  }
...
async function processPromoCode() {
  ...
  // Проверяем промокод NIKOL для входа в админку
  if (promoCode === 'NIKOL') {
    ...
    if (result.success) {
      ...
      closePromoModal();
      updateOrderPrice(); // Обновляем цену с учетом скидки
    } else {
...
// Функция проверки и применения промокода
async function applyPromoCode(promoCode) {
  try {
    // Проверяем промокод в Firestore
    const promoDoc = await firebase.firestore().collection('promocodes').doc(promoCode).get();
    ...
    if (promoData.activations >= promoData.maxActivations) {
    ...
    if (!currentUser || !currentUser.uid) {
      return { success: false, message: 'Необходимо войти в аккаунт для использования промокода' };
    }
    
    const userPromoDoc = await firebase.firestore()
      .collection('promocodes')
      .doc(promoCode)
      .collection('users')
      .doc(currentUser.uid)
    ...
    // Увеличиваем общий счетчик активаций
    await firebase.firestore().collection('promocodes').doc(promoCode).update({
      activations: firebase.firestore.FieldValue.increment(1)
    });
    ...
    return { success: true, message: `Промокод применен! Скидка: ${promoDiscount}₽` };
    
  } catch (error) {
    console.error('Ошибка применения промокода:', error);
    return { success: false, message: 'Ошибка применения промокода' };
  }
}
...
function removePromoCode() {
  ...
  promoDiscount = 0;
  updateOrderPrice();
  showNotification('Промокод отменен', 'info');
}
...
function updateTimeInfo() {
  ...
  if (timeInfo) {
    let promoInfo = '';
    if (activePromoCode) {
      promoInfo = ` | Промокод: ${activePromoCode} (-${promoDiscount}₽)`;
    }
    
    timeInfo.innerHTML = `
      <div style="font-size: 0.9rem; color: #ccc; margin-bottom: 10px;">
        <span style="color: ${isAvailable ? '#4CAF50' : '#F44336'};">●</span>
        МСК: ${timeString} | Базовая цена: ${basePrice}₽ | Доп. опции: 100₽${promoInfo}
      </div>
    `;
  }
  ...
  if (promoInfo && promoDiscountSpan) {
    if (activePromoCode && promoDiscount > 0) {
      promoInfo.style.display = 'block';
      promoDiscountSpan.textContent = promoDiscount;
    } else {
